...
@allure.feature("用户管理")
@allure.story("用户业务流：创建 → 查询 → 修改 → 确认修改 → 删除 ")
@allure.severity(allure.severity_level.CRITICAL)
@pytest.mark.flow
class TestUserFlow:
    """
    用户管理业务流测试
    目标：验证用户的创建 → 查询 → 修改 → 确认修改 → 删除整个流程的正确性
    """

    def test_user_business_flow(self, user_api,user_factory):
        """
        测试步骤：
        1. 创建用户
        2. 修改用户信息
        ...
        user_id = getattr(create_res, "created_userid")
        assert create_res.status_code == 200
        assert create_res.json()["errcode"] == 0

        #2. 查询用户(确认创建成功)
        log.info("Step 2: 查询确认用户创建成功")
