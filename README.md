# 基于pytest的通用接口自动化测试框架

基于 **Python + Pytest + Requests + Allure** 构建的接口自动化测试框架，支持部门、用户、标签、消息等核心模块的自动化测试，包括数据驱动、fixture 工厂管理、业务流测试、异常链路测试、日志收集以及测试数据的自动准备和清理。

## 项目背景

- **项目简介**：基于pytest的通用接口自动化测试框架；测试场景：企业微信通讯录管理和消息发送核心业务场景
