...
---
### 数据库校验(模拟)
由于接口自动化测试无法直接访问真实生产/开发数据库，
本项目中的数据库校验采用 **测试侧模拟数据的方式**：
- 不干扰真实业务数据
- 不依赖后端实现细节
- 重点验证接口行为与数据结果的一致性
#### 实现方式
- 通过 pytest fixture 管理数据库连接生命周期
- 使用独立的 MySQL 测试数据库（不依赖真实后端数据库）
- 在接口调用后，将关键业务字段（如 id、name 等）写入测试表
- 通过 SQL 查询对接口返回结果与数据库中的数据进行一致性校验
